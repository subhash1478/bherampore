...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_5__ionic_native_native_page_transitions__ = __webpack_require__(209);
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
...
var HomePage = /** @class */ (function () {
    function HomePage(menuCtrl, events, nativePageTransitions, utility, navCtrl, navParams, _services, viewCtrl, modalCtrl) {
        ...
        this._services.getBanner().subscribe(function (Response) {
            ...
            for (var index = 0; index < result.length; index++) {
                ...
                if (result[index].bannerindex == 1) {
                    ...
                    val['linkto'] = result[index].linkto.linkto;
                    banner1.push(val);
                }
                if (result[index].bannerindex == 2) {
                    val['image'] = result[index].image;
                    val['bannerindex'] = result[index].bannerindex;
                    banner2.push(val);
                }
                if (result[index].bannerindex == 3) {
                    val['image'] = result[index].image;
                    val['bannerindex'] = result[index].bannerindex;
                    banner3.push(val);
                }
            }
            _this.banner.push(banner1);
    ...
    HomePage.prototype.saveDevicesToken = function () {
        ...
        var devicesid = localStorage.getItem('devices_token');
        if (devicesid != null) {
            var data = {
                devicesid: localStorage.getItem('devices_token'),
                userid: user._id,
            ...
            this._services.saveDevicesToken(data).subscribe(function (response) {
            });
        }
    };
    HomePage.prototype.showPromoterCategory = function (item) {
        ...
        if (item[0].bannerindex == 2) {
            ...
            this.navCtrl.push("PostPage", { id: item_2._id, title: item_2.title });
        }
